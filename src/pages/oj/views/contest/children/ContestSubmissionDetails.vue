      ...
      return {
        columns: [
          ...
          },
          {
            ...
            align: 'center',
            render: (h, params) => {
              // Extract the first float from params.row.output
              let averageWeight = '';
              if (typeof params.row.output === 'string') {
                const match = params.row.output.match(/([0-9]+\.[0-9]+)/);
                if (match) {
                  averageWeight = match[1];
                }
              }
              return h('span', averageWeight)
            }
          }
        ],
        submission: {
